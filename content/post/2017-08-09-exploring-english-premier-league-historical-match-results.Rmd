...
```{r arsenal-points-graph-1, fig.width = 8, fig.height = 4}
data_tidy %>%
  filter(team == "Arsenal",
         season < 2007) %>%
...
```{r arsenal-points-graph-2, fig.width = 8, fig.height = 4}
data_tidy %>%
  filter(team == "Arsenal",
         season >= 2007) %>%
  ggplot(aes(Date, points)) +
...
```{r bar-graph-season-stats, fig.width = 8, fig.height = 4}
season_ending_stats %>%
...
```{r final-points-joyplot, fig.width = 8, fig.height = 8, message = FALSE}
season_ending %>%
  group_by(team) %>%
...
```{r home-adv-graph, fig.width = 8, fig.height = 4}
opening_games %>%
...
```{r opening-win-perc, fig.width = 8, fig.height = 4}
opening_games %>%
  group_by(team) %>%
  #removing teams that have not played at least ten seasons in the premier league
  mutate(num_seasons = n()) %>%
